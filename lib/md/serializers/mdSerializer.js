...
export default class MdSerializer extends Markdown {
  ...
  serialize (value) {
    ...
    return super.serialize(Value.fromJSON(editorValue)).replace(/\\/g, '');
  }
  ...
  serializeNodes = nodes => {
    ...
    let wasImageWrapped = false;

    nodes.forEach(n => {
      if (n.object === 'block') {
        const conversionFn = {
          [SCRIBO_TYPES.BLOCK_QUOTE]: () =>
            (n.type = SERIALIZER_TYPES.BLOCK_QUOTE),
          [SCRIBO_TYPES.NUMBERED_LIST]: () =>
            (n.type = SERIALIZER_TYPES.ORDERED_LIST),
          [SCRIBO_TYPES.CODE_BLOCK]: () => (n.type = SERIALIZER_TYPES.CODE),
          [SCRIBO_TYPES.HEADING_ONE]: () =>
            (n.type = SERIALIZER_TYPES.HEADING1),
          [SCRIBO_TYPES.HEADING_TWO]: () =>
            (n.type = SERIALIZER_TYPES.HEADING2),
          [SCRIBO_TYPES.HEADING_THREE]: () =>
            (n.type = SERIALIZER_TYPES.HEADING3),
          [SCRIBO_TYPES.TABLE_CELL]: () =>
            (n.type = SERIALIZER_TYPES.TABLE_HEAD),
          [SCRIBO_TYPES.IMAGE]: () => {
            // Wrap image block with a paragraph..
            // `slate-md-serializer` expects image blocks to be in this format
            const imageBlock = cloneDeep(n);
            ...
            wasImageWrapped = true;
          }
        }[n.type];

        if (isFunction(conversionFn)) conversionFn();

        // This is to prevent infinite loops during recursion..
        // If this was not here, we'd be constantly running `serializeNodes` on image nodes
        if (wasImageWrapped) return;

        this.serializeNodes(n.nodes);
      } else if (n.object === 'text') {
        // Base case for the recursion -- It is assumed that marks reside only at the base in the slate structure.
  ...
  deserializeNodes = nodes => {
    ...
    nodes.forEach(n => {
      if (n.object === 'block') {
        const conversionFn = {
          [SERIALIZER_TYPES.BLOCK_QUOTE]: () =>
            (n.type = SCRIBO_TYPES.BLOCK_QUOTE),
          [SERIALIZER_TYPES.ORDERED_LIST]: () =>
            (n.type = SCRIBO_TYPES.NUMBERED_LIST),
          [SERIALIZER_TYPES.CODE]: () => (n.type = SCRIBO_TYPES.CODE),
          [SERIALIZER_TYPES.HEADING1]: () =>
            (n.type = SCRIBO_TYPES.HEADING_ONE),
          [SERIALIZER_TYPES.HEADING2]: () =>
            (n.type = SCRIBO_TYPES.HEADING_TWO),
          [SERIALIZER_TYPES.HEADING3]: () =>
            (n.type = SCRIBO_TYPES.HEADING_THREE),
          [SERIALIZER_TYPES.TABLE_HEAD]: () =>
            (n.type = SCRIBO_TYPES.TABLE_CELL),
        ...
        n.marks.forEach(mark => {
          const conversionFn = {
            [SERIALIZER_TYPES.DELETED]: () =>
              (mark.type = SCRIBO_TYPES.STRIKETHROUGH)
          }[mark.type];
