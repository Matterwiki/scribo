...
export function addOrRemoveBlock(value, type) {
    ...
    change.setBlocks(hasBlockInRange ? DEFAULT_NODE : type);

    // Insert empty paragraph block if there is nothing below inserted block
    if (!document.getNextBlock(anchorKey)) change.insertBlock(DEFAULT_NODE);

    change.moveToStartOfPreviousBlock().focus();
