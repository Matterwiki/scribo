 ...
 * TODO We should maybe move custom node specific utils into their plugin implementations. eg. `commands.addEmoji` into `plugins/emoji/index.js`.
 */
export default function CommandsAndQueriesPlugin() {
    return {
        commands: {
             ...
             * @param {*} type
             */
            addOrRemoveList(editor, value, type) {
        ...
        queries: {
             ...
             * @param {*} type
             */
            hasList(editor, value, type) {
                if (!value.blocks.size) return;

                const parent = value.document.getParent(value.blocks.first().key);
                return (
                    editor.hasBlock(value, NODE_TYPES.LIST_ITEM) && parent && parent.type === type
                );
            },
